...
void vault_schedule_renewal (vault_impl_t *vault)
{
  ...
  if (data)
  {
    ...
    else
    {
      ...
      {
        vault->renewjob = iot_schedule_create (vault->scheduler, vault_perform_renewal, NULL, vault, 0, IOT_SEC_TO_NS(wait), 1, vault->thpool, -1);
        iot_schedule_add (vault->scheduler, vault->renewjob);
      }
    }
  }
  else
...
static void vault_fini (void *impl)
{
  ...
  if (vault->renewjob)
  {
    iot_schedule_delete (vault->scheduler, vault->renewjob);
  }
