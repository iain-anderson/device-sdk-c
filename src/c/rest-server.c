...
static bool cors_request_ok (struct MHD_Connection *conn, const char *method, const cors_config *cors)
{
  if (strcmp (cors->allowedorigin, "*"))
  {
    ...
    if (strcmp (cors->allowedorigin, origin))
    {
      return false;
    }
  }
  ...
  *context = 0;
  ...
  if (method == DevSDK_Options && svr->cors.enabled)
  {
    if (cors_request_ok (conn, NULL, &svr->cors))
    {
      ...
      MHD_add_response_header (response, MHD_HTTP_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, svr->cors.allowedorigin);
      MHD_add_response_header (response, "Access-Control-Allow-Credentials", svr->cors.allowcreds);
      MHD_add_response_header (response, "Access-Control-Allow-Methods", svr->cors.allowmethods);
      MHD_add_response_header (response, "Access-Control-Allow-Headers", svr->cors.allowheaders);
  ...
  if (cors_passed)
  {
    MHD_add_response_header (response, MHD_HTTP_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, svr->cors.allowedorigin);
    MHD_add_response_header (response, "Access-Control-Allow-Credentials", svr->cors.allowcreds);
    MHD_add_response_header (response, "Access-Control-Expose-Headers", svr->cors.exposeheaders);
  }
